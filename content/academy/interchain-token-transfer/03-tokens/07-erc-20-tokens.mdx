...
There is only a single native token to a chain. However, to allow to represent a wide range of assets on EVM-chains, the ERC-20 token standard was developed. "ERC" stands for <GlossaryTerm>Ethereum</GlossaryTerm> Request for Comment, and "20" is the proposal identifier.

ERC-20 tokens are <GlossaryTerm>fungible</GlossaryTerm>, meaning each token is identical and can be exchanged on a one-to-one basis. These tokens are typically created and managed through <GlossaryTerm>Smart Contract</GlossaryTerm>s that adhere to the ERC-20 standard, which defines a set of rules and functions that ensure interoperability between different tokens and <GlossaryTerm>decentralized applications</GlossaryTerm> (DApps). 
...
ERC-20 tokens can represent various assets, from digital currencies to tokenized assets, and they play a crucial role in the crypto ecosystem by enabling the creation of decentralized applications with diverse functionalities, such as token sales, <GlossaryTerm>decentralized finance</GlossaryTerm> (DeFi) protocols, and more.
