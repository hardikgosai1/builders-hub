...
```bash
forge create --rpc-url dispatch --private-key $PK contracts/interchain-messaging/incentivize-relayer/receiverWithFees.sol:ReceiverOnSubnet --broadcast
```
```
[⠊] Compiling...
...
```bash
cast send --rpc-url fuji-c --private-key $PK $ERC20_fee_address "approve(address,uint256)" $SENDER_ADDRESS 500000000000000
```
...
```bash
cast send --rpc-url fuji-c --private-key $PK $SENDER_ADDRESS "sendMessage(address, string, address)" $RECEIVER_ADDRESS "Hello" $ERC20_fee_address
```
...
```bash
cast call --rpc-url dispatch $RECEIVER_ADDRESS "lastMessage()(string)"
```
```bash
"Hello"
```
