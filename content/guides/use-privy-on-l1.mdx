...
In this guide, we are using the `Echo L1` as an example <GlossaryTerm>Avalanche L1</GlossaryTerm>. However, you can use any <GlossaryTerm>Avalanche L1</GlossaryTerm> that has a public RPC URL. If the L1 has an explorer page, you can see better what is happening, but it is not required.
...
We've used the user object of Privy to retrieve the user ID and wallet address. To fetch the user's balance, we need to create an HTTP client for our <GlossaryTerm>Avalanche L1</GlossaryTerm>, which we've already defined earlier.
