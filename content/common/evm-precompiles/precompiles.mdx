...
Precompiles can be called from a Solidity <GlossaryTerm>Smart Contract</GlossaryTerm> in the same way, as if they were another contract written in Solidity. The EVM keeps a list of addresses reserved and mapped to the precompiles. When a smart contract calls a function of a contract with an address on that list, the EVM executes the precompile written in Go instead of the smart contract.
For instance, if we mapped the address 0x030...01 to the SHA256 precompile that <GlossaryTerm>Hash</GlossaryTerm>es its input using the SHA256 Hash Function, we can call the Precompile like this:
