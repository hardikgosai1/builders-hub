...
- **Authenticity:** Authenticity ensures that the signature authenticates the origin of the message and verifies that it was indeed created by the claimed sender. A valid signature on a message proves that the signer owns the <GlossaryTerm>Private Key</GlossaryTerm> associated with the <GlossaryTerm>Public Key</GlossaryTerm> used for the signature.
