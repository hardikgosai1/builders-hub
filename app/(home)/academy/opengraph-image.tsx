...
// Regular fonts
const medium = fetch(new URL('../../api/og/Geist-Medium.ttf', import.meta.url)).then((res) =>
  res.arrayBuffer(),
);

const light = fetch(new URL('../../api/og/Geist-Light.ttf', import.meta.url)).then((res) =>
  res.arrayBuffer(),
);

const regular = fetch(new URL('../../api/og/GeistMono-Light.ttf', import.meta.url)).then((res) =>
  res.arrayBuffer(),
);

// Image metadata
export const alt = 'Avalanche Academy';
export const size = {
  width: 1200, 
  height: 630,
};
