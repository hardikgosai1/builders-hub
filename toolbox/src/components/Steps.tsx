...
interface StepGroupType {
    title: string;
    icon: ExoticComponent;
}

export type StepListType = Record<string, StepType>;
...
export function Steps({
    stepGroups, stepList, currentStep, maxAdvancedStep, advanceTo, onReset
}: StepsProps) {
    // ... existing code ...
    const stepsGroupped = Object.entries(stepList).reduce<Record<string, string[]>>((acc, [key, step]) => {
                    ...
                    return (
                        <div key={groupKey} className="">
                            ...
                            <div className="ml-5 pl-4">
                                {stepsGroupped[groupKey]?.map((stepKey) => {
